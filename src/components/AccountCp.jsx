...
export default function AccountPg() {
  ...
  const fetchPosts = useCallback(async ({ queryKey }) => {
    ...
    if (user) {
      ...
      if (url) {
        ...
        return { posts: data.addedPosts || data.bookmarkedPosts, totalPages: data.totalPages, totalPosts: data.totalPosts };
      }
    }
      return { posts: [], totalPages: 0, totalPosts: 0 };
    }, [user, activeTab]);

  const { data: addedPostsData, isLoading: addedPostsLoading } = useQuery(['posts', 'added', user, currentPage], fetchPosts, {
    enabled: activeTab === 'added',
    keepPreviousData: true, // keep old data until new data is fetched
  });

  const { data: bookmarkedPostsData, isLoading: bookmarkedPostsLoading } = useQuery(['posts', 'bookmarks', user, currentPage], fetchPosts, {
    enabled: activeTab === 'bookmarks',
    keepPreviousData: true, // keep old data until new data is fetched
  });
  console.log(bookmarkedPostsData, addedPostsData)
  ...
  return (
    <div className="container mx-auto px-10 lg:px-0 max-w-3xl">
      ...
      <p>Total posts: {activeTab === "bookmarks" ? bookmarkedPostsData?.totalPosts : addedPostsData?.totalPosts}</p>
      <div className="flex flex-col lg:flex-row md:flex-row gap-5 mb-10 mt-10">
        <ButtonGroup variant="text">
          <Button
            className={`rounded-none text-white transition-colors duration-500 capitalize ${activeTab === "bookmarks" ? "!bg-emerald-900" : ""}`}
            onClick={() => setActiveTab("bookmarks")}
          >
            Bookmarks
          </Button>
          <Button
            className={`rounded-none text-white transition-colors duration-500 capitalize ${activeTab === "added" ? "!bg-emerald-900" : ""}`}
      ...
      {activeTab === "bookmarks" && bookmarkedPostsData?.posts && (
      ...
      {activeTab === "added" && addedPostsData?.posts && (
