...
function ApproveTool() {
  ...
  useEffect(() => {
    ...
    axios
      .get(
        `${SV_URL}/update-tool-status?toolId=${toolId}&pending=${pendingStatus}`
      )
      .then((res) => {
        // handle success
  ...
  return (
    <div
      className={`p-4 text-white min-h-screen flex items-center justify-center ${
        pending === "approved" ? "bg-green-500" : "bg-red-500"
      }`}
    >
